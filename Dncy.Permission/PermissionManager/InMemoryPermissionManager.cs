...
namespace Dncy.Permission
{
    ...
    {
        string cacheKey = string.Format(Permission_Grant_CacheKey_Format, providerName, providerKey, name);
    ...
    protected virtual async Task<(string Key, bool IsGranted)> SetCacheItemsAsync(string providerName, string providerKey, string currentName)
    {
        ...
        foreach (var permission in permissions)
        {
            ...
            permissionCached.TryAdd(string.Format(Permission_Grant_CacheKey_Format, providerName, providerKey, permission.Name), isGranted.ToString());
        ...
        return (string.Format(Permission_Grant_CacheKey_Format, providerName, providerKey, currentName), currentResult);
    }
    ...
    {
        var cacheKeys = names.Select(x => string.Format(Permission_Grant_CacheKey_Format, providerName, providerKey, x)).ToList();
    ...
    protected virtual async Task<List<(string Key, bool IsGranted)>> SetCacheItemsAsync(string providerName, string providerKey, List<string> notCacheKeys)
    {
        ...
        {
            bool isGranted = grantedPermissionsHashSet.Contains(permission.Name);
            cacheItems.Add((string.Format(Permission_Grant_CacheKey_Format, providerName, providerKey, permission.Name), isGranted));
        }
